...
NUM_FRAMES=$(gst-launch-1.0 filesrc location="$FILE" ! qtdemux ! h264parse ! nvdec ! fakesink silent=0 -v 2>&1 | grep chain | wc -l)
...
for i in `seq $NUM_INSTANCES` ; do
  CMD="$CMD filesrc location=$FILE ! qtdemux ! h264parse ! nvdec ! fakesink"
done
